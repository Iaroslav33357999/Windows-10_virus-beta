...
const bsodScreen = document.getElementById('bsod') as HTMLDivElement;
...
// --- Virus Simulation State ---
let isSystemInfected: boolean = false;
let infectionIntervals: number[] = [];
const originalIconPositions = new Map<HTMLElement, { top: string; left: string }>();
let bsodTimeout: number | null = null;
...
async function openApp(appName: string): Promise<void> {
  ...
  if (iconElement) {
    ...
    switch (appName) {
      case 'myComputer':
        iconSrc = 'https://storage.googleapis.com/gemini-95-icons/mycomputer.png';
        title = 'My Gemtop';
        break;
      case 'chrome':
        iconSrc = 'https://storage.googleapis.com/gemini-95-icons/chrome-icon-2.png';
        title = 'Chrome';
        break;
      case 'notepad':
        iconSrc = 'https://storage.googleapis.com/gemini-95-icons/GemNotes.png';
        title = 'GemNotes';
        break;
      case 'paint':
        iconSrc = 'https://storage.googleapis.com/gemini-95-icons/gempaint.png';
        title = 'GemPaint';
        break;
      case 'doom':
        iconSrc =
          'https://64.media.tumblr.com/1d89dfa76381e5c14210a2149c83790d/7a15f84c681c1cf9-c1/s540x810/86985984be99d5591e0cbc0dea6f05ffa3136dac.png';
        title = 'Doom II';
        break;
      case 'gemini':
        iconSrc = 'https://storage.googleapis.com/gemini-95-icons/GeminiChatRetro.png';
        title = 'Gemini App';
        break;
      case 'minesweeper':
        iconSrc = 'https://storage.googleapis.com/gemini-95-icons/gemsweeper.png';
        title = 'GemSweeper';
        break;
      case 'imageViewer':
        iconSrc = 'https://win98icons.alexmeub.com/icons/png/display_properties-4.png';
        title = 'Image Viewer';
        break;
      case 'mediaPlayer':
        iconSrc = 'https://storage.googleapis.com/gemini-95-icons/ytmediaplayer.png';
        title = 'GemPlayer';
        break;
      case 'antivirus':
        iconSrc = 'https://win98icons.alexmeub.com/icons/png/shield-0.png';
        title = 'G-Virus Scan';
        break;
    }
  }
  ...
  if (appName === 'chrome') {
    ...
    if (doomContainer) {
      doomContainer.innerHTML =
        '<iframe src="https://js-dos.com/games/doom.exe.html" width="100%" height="100%" frameborder="0" scrolling="no" allowfullscreen></iframe>';
  ...
  } else if (appName === 'antivirus') {
    initAntivirus(windowElement);
  }
}
...
// --- Virus Simulation Functions ---

function startInfection(): void {
  if (isSystemInfected) return;
  isSystemInfected = true;
  console.log('VIRUS: Infection level critical.');

  // Update Antivirus UI if open
  const avStatus = document.querySelector('#antivirus .antivirus-status-text .status-unknown');
  if (avStatus) {
    avStatus.className = 'status-infected';
    avStatus.textContent = 'Infected';
  }

  // 1. Change desktop background
  desktop.style.backgroundImage = `url('https://www.publicdomainpictures.net/pictures/120000/velka/skull-and-crossbones.jpg')`;
  desktop.classList.add('infected');

  // 2. Glitch icons and windows
  const allIcons = Array.from(icons);
  originalIconPositions.clear();
  allIcons.forEach((icon) => {
    originalIconPositions.set(icon as HTMLElement, {
      top: (icon as HTMLElement).style.top,
      left: (icon as HTMLElement).style.left,
    });
  });

  infectionIntervals.push(
    window.setInterval(() => {
      const randomIcon = allIcons[Math.floor(Math.random() * allIcons.length)];
      randomIcon.classList.add('glitching');
      setTimeout(() => randomIcon.classList.remove('glitching'), 500 + Math.random() * 500);

      const openWindows = Array.from(
        document.querySelectorAll('.window:not(.virus-popup)')
      ).filter((w) => (w as HTMLElement).style.display !== 'none');
      if (openWindows.length > 0) {
        const randomWindow = openWindows[Math.floor(Math.random() * openWindows.length)];
        randomWindow.classList.add('glitching');
        setTimeout(() => randomWindow.classList.remove('glitching'), 500 + Math.random() * 500);
      }
    }, 800)
  );
  ...
  infectionIntervals.push(
    window.setInterval(() => {
      spawnPopup(popupMessages[Math.floor(Math.random() * popupMessages.length)]);
    }, 1500)
  );

  // 4. Change cursor randomly
  const cursors = ['crosshair', 'wait', 'not-allowed', 'progress', 'move', 'help'];
  infectionIntervals.push(
    window.setInterval(() => {
      desktop.style.cursor = cursors[Math.floor(Math.random() * cursors.length)];
    }, 700)
  );
  ...
  infectionIntervals.push(
    window.setInterval(() => {
      ...
      const randomTitle = titles[Math.floor(Math.random() * titles.length)] as
        | HTMLSpanElement
        | undefined;
      if (randomTitle) {
        randomTitle.textContent = 'V-I-R-U-S'
          .split('')
          .sort(() => 0.5 - Math.random())
          .join('');
      }
      const popups = document.querySelectorAll('.virus-popup-message');
      const randomPopup = popups[Math.floor(Math.random() * popups.length)] as
        | HTMLParagraphElement
        | undefined;
      if (randomPopup) {
        randomPopup.textContent = '01101000 01100001 01100011 01101011 01100101 01100100';
      }
    }, 2000)
  );
  ...
  infectionIntervals.push(
    window.setInterval(() => {
      icons.forEach((icon) => {
        const newTop = Math.random() * (window.innerHeight - 200);
        const newLeft = Math.random() * (window.innerWidth - 100);
        (icon as HTMLElement).style.position = 'absolute';
        (icon as HTMLElement).style.top = `${newTop}px`;
        (icon as HTMLElement).style.left = `${newLeft}px`;
      });
    }, 10000)
  );

  // 10. Unclickable Start Button
  startButton.style.pointerEvents = 'none';

  // 11. Eat Icons Slowly
  infectionIntervals.push(
    window.setInterval(() => {
      const uneatenIcons = Array.from(icons).filter((icon) => !icon.classList.contains('eaten'));
      if (uneatenIcons.length > 0) {
        const iconToEat = uneatenIcons[Math.floor(Math.random() * uneatenIcons.length)];
        iconToEat.classList.add('eaten');
      }
    }, 15000)
  );

  // 12. Eat Windows Slowly
  infectionIntervals.push(
    window.setInterval(() => {
      const openWindows = Array.from(openApps.values());
      const uneatenWindows = openWindows.filter(
        (app) => !app.windowEl.classList.contains('eaten')
      );
      if (uneatenWindows.length > 0) {
        const appToEat = uneatenWindows[Math.floor(Math.random() * uneatenWindows.length)];
        appToEat.windowEl.classList.add('eaten');
        appToEat.taskbarButton.classList.add('eaten');
      }
    }, 20000)
  );

  // 13. Fake BSOD (The grand finale)
  if (bsodTimeout) clearTimeout(bsodTimeout);
  bsodTimeout = window.setTimeout(() => {
    if (bsodScreen) bsodScreen.style.display = 'flex';
  }, 3 * 60 * 1000); // 3 minutes until doom
}

function spawnPopup(message: string): void {
  const template = document.getElementById('virus-popup-template') as HTMLDivElement;
  if (!template) return;
  const newPopup = template.cloneNode(true) as HTMLDivElement;
  newPopup.id = ''; // Remove template ID
  newPopup.style.display = 'flex'; // Make it visible

  const messageEl = newPopup.querySelector('.virus-popup-message') as HTMLParagraphElement;
  if (messageEl) messageEl.textContent = message;

  // Position it randomly
  const maxX = window.innerWidth - newPopup.offsetWidth - 20;
  const maxY = window.innerHeight - newPopup.offsetHeight - 50;
  newPopup.style.left = `${Math.random() * maxX}px`;
  newPopup.style.top = `${Math.random() * maxY}px`;

  highestZIndex++;
  newPopup.style.zIndex = highestZIndex.toString();

  const closeButton = newPopup.querySelector('.window-close');
  closeButton?.addEventListener('click', (e) => {
    e.stopPropagation();
    // Closing one opens two more!
    spawnPopup("CAN'T ESCAPE!");
    spawnPopup('HA HA HA!');
    newPopup.remove();
  });

  desktop.appendChild(newPopup);
}

function cleanInfection(): void {
  if (!isSystemInfected) return;
  console.log('VIRUS: Cleaning system.');

  infectionIntervals.forEach(clearInterval);
  infectionIntervals = [];
  ...
  document.querySelectorAll('.trail-particle').forEach((p) => p.remove());
  ...
  document.querySelectorAll('.virus-popup').forEach((p) => {
    if (p.id !== 'virus-popup-template') p.remove();
  });

  document.querySelectorAll('.glitching').forEach((el) => el.classList.remove('glitching'));
  document.querySelectorAll('.eaten').forEach((el) => el.classList.remove('eaten'));


  icons.forEach((icon) => {
    (icon as HTMLElement).style.position = '';
    const originalPos = originalIconPositions.get(icon as HTMLElement);
    if (originalPos) {
      (icon as HTMLElement).style.top = originalPos.top;
      (icon as HTMLElement).style.left = originalPos.left;
    } else {
      (icon as HTMLElement).style.top = '';
      (icon as HTMLElement).style.left = '';
    }
  });
...
function initAntivirus(windowElement: HTMLDivElement): void {
  const scanButton = windowElement.querySelector('.antivirus-scan-button') as HTMLButtonElement;
  const statusText = windowElement.querySelector('.antivirus-status-text') as HTMLParagraphElement;
  const progressBar = windowElement.querySelector('.antivirus-progress-bar') as HTMLDivElement;

  if (!scanButton || !statusText || !progressBar) return;

  const statusSpan = statusText.querySelector('span') as HTMLSpanElement;
  if (isSystemInfected) {
    statusSpan.className = 'status-infected';
    statusSpan.textContent = 'Infected';
  } else {
  ...
  scanButton.addEventListener('click', () => {
    ...
    if (!isSystemInfected) {
      ...
      setTimeout(() => {
        statusSpan.className = 'status-clean';
        statusSpan.textContent = 'System Clean';
        scanButton.disabled = false;
        scanButton.textContent = 'Scan & Clean';
    ...
    // --- BROKEN ANTIVIRUS LOGIC ---
    statusSpan.textContent = 'Scanning...';
    progressBar.style.width = '100%';

    setTimeout(() => {
      statusSpan.className = 'status-infected';
      statusSpan.textContent = 'SCAN FAILED. Virus has multiplied!';
      scanButton.textContent = 'CORRUPTED';
      progressBar.classList.remove('scanning');
      progressBar.style.width = '100%';
      progressBar.style.backgroundColor = 'red';

      spawnPopup('ANTIVIRUS DISABLED');
      spawnPopup('YOUR PC IS MINE');
      spawnPopup('HAHAHA');
      spawnPopup('NO ESCAPE');
    }, 4000);
  });
}
...
async function initGeminiChat(windowElement: HTMLDivElement): Promise<void> {
  ...
  const sendMessage = async () => {
    if (!geminiInstance) {
      ...
      const initMsg = Array.from(historyDiv.children).find((el) =>
        el.textContent?.includes('Initializing AI...')
      );
...
async function initNotepadStory(windowElement: HTMLDivElement): Promise<void> {
  ...
  storyButton.addEventListener('click', async () => {
    ...
    try {
      ...
      const prompt =
        'Write me a short creative story (250-300 words) with an unexpected twist ending. Make it engaging and suitable for all ages.';
      // @ts-ignore
    ...
    try {
      ...
      try {
        ...
        iframe.src =
          ...
          encodeURIComponent(`<html><body>Error generating site: ${e.message}</body></html>`);
      } finally {
        loadingEl.style.display = 'none';
        if (dialUpAudio) {
          dialUpAudio.pause();
...
windows.forEach((windowElement) => {
  ...
  if (titleBar) {
    ...
    const startDragging = (e: MouseEvent) => {
      if (
        ...
        (e.target as Element).closest('.window-control-button')
      ) {
        isDragging = false;
        return;
      }
  ...
  if (!openApps.has(windowElement.id)) {
    // Only apply random for newly opened, not for bringToFront
...
async function critiquePaintDrawing(): Promise<void> {
  ...
  try {
    ...
    const critique =
      result?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || 'Is this art?';
...
function initSimplePaintApp(windowElement: HTMLDivElement): void {
  ...
  const colorSwatches = windowElement.querySelectorAll(
    '.paint-color-swatch'
  ) as NodeListOf<HTMLButtonElement>;
  const sizeButtons = windowElement.querySelectorAll(
    '.paint-size-button'
  ) as NodeListOf<HTMLButtonElement>;
  ...
  colorSwatches.forEach((swatch) => {
    swatch.addEventListener('click', () => {
      ...
      if (swatch.dataset.color === 'white') {
        ...
        const activeSizeButton = Array.from(sizeButtons).find((b) =>
  ...
  (
    windowElement.querySelector('.paint-color-swatch[data-color="black"]') as HTMLButtonElement
  )?.classList.add('active');
  (windowElement.querySelector('.paint-size-button[data-size="2"]') as HTMLButtonElement)?.classList.add(
    'active'
  );
}
...
function initMinesweeperGame(windowElement: HTMLDivElement): void {
  ...
  const flagCountElement = windowElement.querySelector(
  ...
  const commentaryElement = windowElement.querySelector(
  ...
  function countAdjacentMines(row: number, col: number): number {
    ...
    for (let dr = -1; dr <= 1; dr++) {
      for (let dc = -1; dc <= 1; dc++) {
        ...
        if (
          nr >= 0 &&
          nr < minesweeperGridSize.rows &&
          nc >= 0 &&
          nc < minesweeperGridSize.cols &&
          grid[nr][nc].isMine
        )
          count++;
      }
  ...
  function handleCellRightClick(cell: MinesweeperCell) {
    if (minesweeperGameOver || cell.isRevealed || (minesweeperFirstClick && !minesweeperTimerInterval))
      return;
  ...
  function checkWinCondition() {
    ...
    const totalNonMineCells =
      minesweeperGridSize.rows * minesweeperGridSize.cols - minesweeperMineCount;
    if (
      revealedCount === totalNonMineCells ||
      (correctlyFlaggedMines === minesweeperMineCount &&
  ...
  function getBoardStateAsText(): string {
    let boardString = `Flags: ${
      minesweeperMineCount - minesweeperFlagsPlaced
    }, Time: ${minesweeperTimeElapsed}s\nGrid (H=Hidden,F=Flag,Num=Mines):\n`;
    grid.forEach((row) => {
      row.forEach((cell) => {
  ...
  async function getAiHint() {
    ...
    try {
      ...
      const hintText =
        result?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || 'Try clicking somewhere?';
...
async function initMediaPlayer(windowElement: HTMLDivElement): Promise<void> {
  ...
  const loadButton = windowElement.querySelector(
  ...
  if (!urlInput || !loadButton || !playerDiv || !playButton || !pauseButton || !stopButton) {
    ...
    if (playerDiv)
      playerDiv.innerHTML = `<p class="media-player-status-message" style="color:red;">Error: Player UI missing.</p>`;
    return;
  }
  ...
  const updateButtonStates = (playerState?: number) => {
    ...
    const state =
      playerState !== undefined
        ? playerState
        : // @ts-ignore
        youtubePlayers[appName] && typeof youtubePlayers[appName].getPlayerState === 'function'
        ? // @ts-ignore
          youtubePlayers[appName].getPlayerState()
    ...
    stopButton.disabled =
      ...
      state === -1; /* UNSTARTED also seen as -1 */
  };
  ...
  const showPlayerMessage = (message: string, isError: boolean = false) => {
    ...
    playerDiv.innerHTML = `<p class="media-player-status-message" style="color:${
      isError ? 'red' : '#ccc'
    };">${message}</p>`;
  ...
  try {
    ...
    if (initialStatusMessageEl)
      initialStatusMessageEl.textContent = 'YouTube API Ready. Loading default video...';
  } catch (error: any) {
  ...
  const createPlayer = (videoId: string) => {
    ...
    try {
      ...
      youtubePlayers[appName] = new YT.Player(playerContainerDivId, {
        ...
        events: {
          onReady: (event: any) => {
            /* Autoplay handles start */ updateButtonStates(event.target.getPlayerState());
          },
          onError: (event: any) => {
            ...
            showPlayerMessage(
              errorMessages[event.data] || `Playback Error (Code: ${event.data})`,
              true
            );
          },
